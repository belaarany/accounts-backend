	...
	return (request: express.Request, response: express.Response, next: express.NextFunction): void => {
		...
		Promise.resolve()
			...
			.catch((error: { schema: "params" | "body"; errors: Array<ValidationError> }) => {
				let parsedErrors: object = parseErrors(error.errors, errorResponse, undefined, undefined, error.schema)

				winston.debug(
					`Request validation failed --> schema: '${error.schema}' | errors: '${JSON.stringify(
						parsedErrors,
					)}'`,
				)
...
const validateSchema = (type: any, plain: Array<{}>, schema: "params" | "body"): Promise<any> => {
	return new Promise(
		(
			resolve: () => void,
			reject: (error: { schema: "params" | "body"; errors: Array<ValidationError> }) => void,
		) => {
	...
	errors.forEach((error: ValidationError) => {
		if (error.constraints && error.hasOwnProperty("constraints") === true) {
			...
			errorResponse.addError({
				...
				reason:
					schema === "params"
						? ErrorReason.Request.INVALID_PARAMETER_PROPERTY
						: ErrorReason.Request.INVALID_BODY_PROPERTY,
